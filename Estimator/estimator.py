...
from .utils import convert_nest_to_fused_graph, initialize_accelerator, reset_accelerator, get_engine_type
...
from .utils import nest_coretype_mapping, e_tuple
...
def populate_estimates(tmpc_models, max_tmp_width, micro_batch_size, sequence_length, ep_degree):
    setup_architecure()
...
def append_latency_estimates(model):
    ...
    nest_graph = model.get_nest_graph().get_graph()
    fusedgraphs = [convert_nest_to_fused_graph(graph) for graph in graphs]
            ...
            for node in list(fusedgraph.nodes.values()):
                ...
                if (node.node_desc == "AllReduceBwd" or node.node_desc == "AllReduceFwd" or node.node_desc == "AllReduce"):
                    e = allreduce_estimator(
                        nest_graph.nodes[node.node_id], model.tmp_width, node.node_desc, bandwidth, bytes_per_element,)
                    ...
                    e = alltoall_estimator(
                        nest_graph.nodes[node.node_id], model.ep_degree, node.node_desc, bandwidth, bytes_per_element,)
                    ar_estimates[str(node.node_id)] = e
                elif (node.node_desc == "Gather"):
                    e = gather_estimator(
                        nest_graph.nodes[node.node_id], model.ep_degree, node.node_desc, bandwidth, bytes_per_element,)
                    ar_estimates[str(node.node_id)] = e
