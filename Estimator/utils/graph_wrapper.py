...
class FusedNodePlus(FusedNode):
    def __init__(self, id, nest_node) -> None:
        op = (nest_node.op).lower()

        if op not in nest_op_mapping.keys():
            raise TypeError(
                "Type 2: Nest to Fused operator mapping not available for op", op,
            )

        node_desc = nest_op_mapping[op]
        activation_size = nest_node.activation_size
        parameter_size = nest_node.parameter_size
        stage_id = None
        output_act = nest_node.activation_shape
        if nest_node.saved_tensor_shape[-1]:
            saved_tensors = nest_node.saved_tensor_shape
        else:
            saved_tensors = nest_node.saved_tensor_shape[:-1]
        parameter = nest_node.parameter_shape
        kernel_size = nest_node.kernel_size
        stride = nest_node.stride
        dilation = nest_node.dilation
        padding = nest_node.padding
        contiguous = nest_node.contiguous
        ...
        self.layer_id = nest_node.layer_id
        self.name = nest_node.name
...
def convert_nest_to_fused_node(nest_node, id):
    return FusedNodePlus(id, nest_node)
...
def convert_nest_to_fused_graph(graph, mode="fwd"):
