...
nest_coretype_mapping = {"Tensor Core": "TC", "Vector Core": "VC",
...
def allreduce_estimator(nest_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    from ..architecture import frequency
    start = time.time()

    tensorsize = nest_node["node"].get_activation_size() * bytes_per_element
    ...
    if(num_devices == 1):
        ...
        num_cycles = get_cycles(tensorsize, num_devices, "AllReduce")
    end = time.time()

    if allreduce_type == "AllReduceFwd" or allreduce_type == "AllReduce":
        efwd = e_tuple(num_cycles/frequency, 0, 0, end - start,)
        ebwd = e_tuple(0, 0, 0, end - start,)
    elif allreduce_type == "AllReduceBwd":
        efwd = e_tuple(0, 0, 0, end - start,)
        ebwd = e_tuple(num_cycles/frequency, 0, 0, end - start,)

    return {"fwd": efwd._asdict(), "bwd": ebwd._asdict()}

def alltoall_estimator(nest_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    from ..architecture import frequency
    start = time.time()
    ...
    if(num_devices == 1):
        ...
        num_cycles = get_cycles(tensorsize, num_devices, "AllToAll")
...
def gather_estimator(nest_node, num_devices, allreduce_type, bandwidth, bytes_per_element):
    start = time.time()
...
def get_flops(fusedgraph):
    ...
    for node in list(fusedgraph.nodes.values()):
        core_type = nest_coretype_mapping[get_engine_type(
            node.node_desc)]
