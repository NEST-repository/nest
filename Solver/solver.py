...
from Estimator import bws, latency_per_level, devices_per_level
from Estimator import frequency
...
def run_solver(models_info, micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree):
    ...
    while not converged:
        for model_name, model_info in models_info.items():
            ...
            out = solve(tmpc_models, latency_estimates, next_cc,
                        micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree)
...
# executes per model
def solve(models, latency_estimates, cc, micro_batch_size, max_tmp_width, sequence_length, activation_recomputation, hbm_size, ep_degree):
    ...
    return device_placement(model_name, layer_state, micro_batch_size, max_tmp_width, sequence_length, utilization, activation_recomputation, hbm_size, ep_degree)


def device_placement(model_name, layer_state, micro_batch_size, max_tmp_width, sequence_length, utilization, activation_recomputation, hbm_size, ep_degree):
    ...
    out_file = generate_out_filename(
        model_name, "json", micro_batch_size, max_tmp_width, sequence_length, "out", ep_degree=ep_degree)
    ...
    # status = subprocess.call(
    #     ["./Solver/device_placement/device_placement", in_filepath, out_filepath])
