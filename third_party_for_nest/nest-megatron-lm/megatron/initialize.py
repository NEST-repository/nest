...
def _compile_dependencies():
    ...
    # TODO: change for nest, torch.distributed cannot be used with a single gpu
    ...
    # TODO; change for nest, cannot have torch.distributed on a single gpu
    # if torch.distributed.get_rank() == 0:
    if args.rank == 0:
        start_time = time.time()
        print('> compiling and loading fused kernels ...', flush=True)
        fused_kernels.load(args)
        # TODO: change for nest, we do not need a barrier for the trace
...
def _initialize_distributed():
    ...
    # TODO: change for nest, getting the world size from args
    #device_count = torch.cuda.device_count()
    device_count = args.world_size
    ...
    # TODO: change for nest; do not need to initialize the process group
...
def _warmup_jit_function():
    ...
    residual = torch.rand((seq_length, args.micro_batch_size, args.hidden_size),
                          dtype=dtype, device='cuda')
    bias = torch.rand((args.hidden_size), dtype=dtype,
                      device='cuda').expand_as(residual)
    dropout_rate = 0.1
    # Warmup JIT fusions with the input grad_enable state of both forward
    # prop and recomputation
    for input_grad, bias_grad, residual_grad in zip([False, True], [True, True], [True, True]):
        ...
        for _ in range(5):
            output = bias_dropout_add_fused_train(
                input, bias, residual, dropout_rate)
