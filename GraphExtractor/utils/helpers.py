...
megatron_language_models = ["megatronbert", "megatrongpt3",
                            "megatrongpt2-xl", "megatrongpt2-54", "megatrongpt2-72", "mixtral"]
llama_models = ["llama2", "llama-70", "llama3"]
mixtral_models = ["mixtral"]
...
def generate_out_filename(model_name, ext, micro_batch_size=1, tmp_width=1, sequence_length=1, module_type="", ep_degree=1):
    ...
    if model_name in megatron_language_models:
        out_file_name = (str(model_name) + "_mbs_" + str(micro_batch_size) + "_sl_" + str(
            sequence_length) + "_tmp_" + str(tmp_width) + "_ep_" + str(ep_degree) + module_type + "." + str(ext))
...
def get_out_filepath_from_modelname(model_name, micro_batch_size=1, tmp_width=1, sequence_length=1, module_type="", ep_degree=1):
    ...
    out_file_name = generate_out_filename(
        model_name, "pickle", micro_batch_size, tmp_width, sequence_length, module_type, ep_degree)
...
def store_obj_to_file(model_name, obj, micro_batch_size=1, tmp_width=1, sequence_length=1, module_type="", ep_degree=1):
    out_file_path = get_out_filepath_from_modelname(
        model_name, micro_batch_size, tmp_width, sequence_length, module_type,ep_degree)
...
def load_obj_from_file(model_name, micro_batch_size=1, tmp_width=1, sequence_length=1, module_type="",ep_degree=1):
    out_file_path = get_out_filepath_from_modelname(
        model_name, micro_batch_size, tmp_width, sequence_length, module_type, ep_degree
    )
