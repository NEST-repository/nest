...
from GraphExtractor.models import MixtralIR
...
def extract_graph(model_name, max_tmp_width=1, micro_batch_size=1, sequence_length=64, force_reextract_model=False,ep_degree=1):
    def extract_model_from_file(tmp_width, ep_degree=1):
        if not force_reextract_model:
            model = load_obj_from_file(
                model_name, micro_batch_size, tmp_width, sequence_length,ep_degree=ep_degree)

            if check_model(model, BaseModelIR, supported_models):
                model_memory = model.nestgraph.get_memory_footprint()
    ...
    if model_name in bert_models:
        ...
        mixtral = extract_model_from_file(max_tmp_width, ep_degree)

        if mixtral:
            return [mixtral]

        mixtral = MixtralIR(model_name, ep_degree=ep_degree)
